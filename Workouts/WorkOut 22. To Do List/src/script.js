    ...
    <div class="seccionTiemposPomodoro">
       <h6>Duración del pomodoro</h6>  
        <div class="botonesTiempos">                       
           ...
           <button class="botonTiempoPomodoro" value="25" id="duracion3">25:00</button>
           <label for="">
...
function prueba(){
    if (timeLeft >= 0 ) {
        ...
        if(isPlaying === true){
            ...
            timeLeft >= 0 ? document.getElementById("base-timer-label").innerHTML = formatTime(timeLeft) : (coutPomodoros(), localStorage.setItem("PomodoroFinalizado", 1), playAlert(), location.reload());
        }        
    }
}
let snd = new Audio("../audio/alarma.mp3")
function playAlert(){
    localStorage.getItem("PomodoroFinalizado") != null ? (snd.play(), alert("Pomodoro Finalizado")) : null;
}
