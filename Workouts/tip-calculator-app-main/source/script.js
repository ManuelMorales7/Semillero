...
for(let i=0; i<buttons.length-1; i++){  
    ...
    buttonsValueP = buttons[i].value;
    buttonValue = buttonsValueP.substring(0, buttonsValueP.length - 1);
    if(i+2 === buttons.length){
      buttonValue = buttons[i+1].value;
    }
  })
  
}

function CalcularTip(billF){  
  
  let billValue = bill.value.trim();
  let peopleValue = numberPeople.value.trim();

  if(billValue === '' && peopleValue === ''){
    //messagge Error
  }else{
    tipValue = billValue*billF/100;
    finalTip.innerHTML = "$" + tipValue;
    amount = tipValue+billValue;
